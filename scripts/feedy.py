...
CSV_FILE = os.path.join(desktop_path, 'macros.csv')
...
class MacroApp(ttk.Window):
    ...
    def _setup_ui(self):
        ...
        self.status_bar = ttk.Label(self, text=f"Gotowy. Plik: {CSV_FILE} | Naciśnij F12, aby zatrzymać makro.", relief=SUNKEN, anchor=W, padding=5)
    ...
    def _load_macros_from_file(self):
        ...
        if not os.path.exists(CSV_FILE):
            with open(CSV_FILE, 'w', newline='', encoding='utf-8') as f:
                writer = csv.DictWriter(f, fieldnames=['nazwa_makra', 'hotkey', 'akcje'])
                writer.writeheader()
            return macros
            ...
            with open(CSV_FILE, 'r', newline='', encoding='utf-8') as f:
                ...
                if 'akcje' in header and 'hotkey' in header:
                    ...
                    Messagebox.show_info(
                        "Wykryto stary format pliku `macros.csv`. Został on wczytany poprawnie. Przy następnym zapisie, plik zostanie automatycznie zaktualizowany do nowego formatu.",
                        "Aktualizacja formatu pliku"
                    )

        except Exception as e:
             Messagebox.show_error(f"Błąd odczytu lub przetwarzania pliku {CSV_FILE}.\nPlik może być uszkodzony.\nBłąd: {e}", "Błąd pliku")
        return macros
    ...
    def _save_macros_to_file(self):
        try:
            with open(CSV_FILE, 'w', newline='', encoding='utf-8') as f:
                header = ['nazwa_makra', 'hotkey', 'akcje']
                ...
                for name, data in sorted(self.macros.items()):
                    writer.writerow({
                        'nazwa_makra': name,
                        'hotkey': data.get('hotkey', HOTKEY_NONE),
                        'akcje': json.dumps(data.get('actions', []))
                    })
            return True
        except Exception as e:
            Messagebox.show_error(f"Błąd zapisu do pliku {CSV_FILE}.\n{e}", "Błąd zapisu")
            return False
